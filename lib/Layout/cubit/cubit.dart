...
emit(DataLoadingState());
  ...
  Future<void> saveHistory(payed,context,cartid) async {
      DateFormat formatter = DateFormat('dd/MM/yyyy');
    ...
    HistoryModel s=HistoryModel(username: userData?.name, cartid: cartid, products: cart2, payed: payed, totalcost: f, date: formattedDate,clientName: client?.name,clientPhoneNumber:client?.phoneNumber ,time:formattedTime );
    s.toMap(context);
      await getHistory();
      editQuantity(cart2);
    isChecked=false;
